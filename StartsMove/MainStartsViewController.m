...
#import "MainStartsViewController.h"
#import "ImageView.h"
#import "Ball.h"
#define SCREEN_WIDTH [[UIScreen mainScreen] bounds].size.width
#define SCREEN_HEIGHT [[UIScreen mainScreen] bounds].size.height

@interface MainStartsViewController()
...
@implementation MainStartsViewController

float height = 450.0f;
float offy = 0.0;
float progress = 0.0;
float tempprogress = 0.0;
float velocity = 0.0;
...
- (void)addBalls{
    ...
    for (int i = 0; i<[self.imageArray count]; i++) {
        Ball *ball = [[Ball alloc] initWithFrame:CGRectMake(0, 0, 120, 120)];
        ball.layer.cornerRadius = CGRectGetWidth(ball.bounds)/2;
        ball.image = [UIImage imageNamed:@"gameBall"];
        ball.center = _top3;
        ball.index = i;
...
-(void)resetTransparency{
    ...
    Ball *top3 = self.balls[[self preIndex:[self preIndex:[self preIndex:self.mCurrentIndex]]]];
    mid.transparency = 1;
    bottom1.transparency = 0.6;
    bottom2.transparency = 0;
    top1.transparency = 0.5;
    top2.transparency = 0.2;
    top3.transparency = 0;
    
    mid.alpha = mid.transparency;
    bottom1.alpha = bottom1.transparency;
    bottom2.alpha = bottom2.transparency;
    top1.alpha = top1.transparency;
    top2.alpha = top2.transparency;
    top3.alpha = top3.transparency;
    
}
-(void)refleshTransparency:(float)progress{
...
//    top3.alpha = 1.0;
    ...
    if (progress>=0) {
        float offScale = mid.transparency-bottom1.transparency;
        mid.alpha = mid.transparency-progress*offScale;
        
        float offScale2 = bottom1.transparency-bottom2.transparency;
        bottom1.alpha = bottom1.transparency-progress*offScale2;
        
        
        bottom2.alpha = 0.0;
        
        float offScale4 = mid.transparency-top1.transparency;
        top1.alpha = top1.transparency+progress*offScale4;
        
        float offScale5 = top1.transparency-top2.transparency;
        top2.alpha = top2.transparency+progress*offScale5;
        
        float offScale6 = top2.transparency-top3.transparency;
        top3.alpha = top3.transparency+progress*offScale6;
        
    }else{
        float offScale = mid.transparency-top1.transparency;
        mid.alpha = mid.transparency-fabsf(progress)*offScale;
        
        float offScale2 = mid.transparency-bottom1.transparency;
        bottom1.alpha = bottom1.transparency+fabsf(progress)*offScale2;
        
        float offScale3 = bottom1.transparency-bottom2.transparency;
        bottom2.alpha = bottom2.transparency+fabsf(progress)*offScale3;
        
        float offScale4 = top1.transparency-top2.transparency;
        top1.alpha = top1.transparency-fabsf(progress)*offScale4;
        
        float offScale5 = top2.transparency-top3.transparency;
        top2.alpha = top2.transparency-fabsf(progress)*offScale5;
...
- (void)touchDown:(UIControl *)sender {
    offy = tempprogress*height;
    self.ballVelocity = CGPointMake(0, 0);
    NSLog(@"offy = %f tempprogress = %f",offy,tempprogress);
    tempprogress = 0.0;
    progress = 0.0;
    self.timeOffset = 0.0;
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (offy>=0) {
        if (offy>height) {
            ...
            self.timer = nil;
            [self resetIndex];
            offy = (int)offy%(int)height;
        }
    }else{
        if (fabsf(offy)>height) {
            tempprogress = 0.0;
            self.timeOffset = 0.0;
            [self.timer invalidate];
            self.timer = nil;
    ...
    float p = (float)offy/height;
    progress = p;
    [self refleshBallsPosition:p];
    [self refleshScale:p];
    [self refleshTransparency:p];
    NSLog(@"offy = %f translation.y = %f  translation.x = %f  p = %f",offy,translation.y,translation.x,p);
    if(recognizer.state == UIGestureRecognizerStateEnded) {
        if (offy!=height&&offy!=0) {
            [self animate];
        }
    }
...
//    self.duration = 0.5*(1-fabsf(progress)); TODO 这里可能需要判断位置，向下和向上
    //下半区
    if (progress>=0) {
        //向下
        if (self.ballVelocity.y>=0.0) {
           self.duration = 0.5*(1-fabsf(progress));
            
        }else{
            self.duration = 0.5*fabsf(progress);//  TODO 当progress为0时会崩溃 现在已经处理这个问题，
        }
        
    }else{
        //向下
        if (self.ballVelocity.y>=0.0) {
            self.duration = 0.5*fabsf(progress);
        }else{
            self.duration = 0.5*(1-fabsf(progress));
...
- (void)step:(CADisplayLink *)timer
{
    ...
    if (progress>=0) {
        //向下
        if (self.ballVelocity.y>=0.0) {
            float offP = 1-progress;
            tempprogress = progress+time*offP;
        ...
        //向下
        if (self.ballVelocity.y>=0.0) {
            float offP = fabsl(progress);
            tempprogress = progress+time*offP;
            
        }else{
            float offP = 1-fabsl(progress);
            tempprogress = progress-time*offP;
    ...
    [self refleshBallsPosition:tempprogress];
    [self refleshScale:tempprogress];
    [self refleshTransparency:tempprogress];
    NSLog(@"self.timeOffset is = %f  tempprogress is %f  progress is %f",self.timeOffset,tempprogress,progress);
    ...
    if (self.timeOffset >= self.duration) {
        ...
        if(fabsf(progress)>=0.8&&fabsf(self.ballVelocity.y)>=800.0){
            progress = 0.0;
            [self animate];
        }
        progress = 0.0;
